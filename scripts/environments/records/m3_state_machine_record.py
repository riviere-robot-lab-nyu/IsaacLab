...
def main():
    ...
    if env_cfg.write_image_to_file:
        for dir_path in ["./images/wrist", "./images/base"]:
            os.makedirs(dir_path, exist_ok=True)
            for f in glob.glob(os.path.join(dir_path, "*.png")):
                os.remove(f)

    # Set initial robot base position, but will randomize around it when episode starts
    ...
    with contextlib.suppress(KeyboardInterrupt):
        while simulation_app.is_running():
            ...
            print("grip action: ", state_machine.grip)
            ...
            # print("state dim: ", torch.cat([robot.data.joint_pos[:, :-1], root_state], dim=1).shape)
