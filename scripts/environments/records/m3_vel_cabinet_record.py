...
from isaaclab_tasks.direct.rrl_mobile_manipulation.rrl_m3_vel_cabinet_camera import rrl_m3_vel_cabinet_camera_env, rrl_m3_vel_cabinet_camera_env_cfg
from isaaclab.utils.math import quat_apply_inverse
from isaaclab.controllers import DifferentialIKController, DifferentialIKControllerCfg
from isaaclab.managers import SceneEntityCfg
from isaaclab.utils.math import subtract_frame_transforms, combine_frame_transforms
from isaaclab.utils.math import matrix_from_quat, quat_from_euler_xyz, quat_mul, quat_apply
...
def get_velocity(state, goal, kp=0.5):
    quat = state[:, 3:7]
    w = quat[:, 0]
    x = quat[:, 1]
    y = quat[:, 2]
...
def main():
    env_cfg = rrl_m3_vel_cabinet_camera_env_cfg.M3VelCabinetCameraEnvCfg()
    ...
    if env_cfg.write_image_to_file:
        for dir_path in ["./images_vel/wrist", "./images_vel/base"]:
    ...
    if cabinet_cfg is not None:
        for actuator_name in cabinet_cfg.actuators.keys():
            cabinet_cfg.actuators[actuator_name].stiffness = 0.0
            cabinet_cfg.actuators[actuator_name].effort_limit_sim = 0.001
    else:
    ...
    dataset_file_path = f"./datasets/sm_dataset_vel_{args_cli.num_demos}_episodes.hdf5"
    ...
    while simulation_app.is_running():
        ...
        vel = get_velocity(root_state, state_machine.robot_goals)
        actions[:, 9] = vel[:, 2]
        vel[:, 2] = 0.0
        vel = quat_apply_inverse(root_state[:,3:7],vel )
        actions[:, 7:9] = vel[:,:2]
        actions[:, :6] = state_machine.joint_goals[:, :6] - robot.data.joint_pos[:,:6]
        is_open_mask = (state_machine.grip[:, 0] == 0)
        actions[:,6] = -1.0
        actions[is_open_mask, 6] = 1.0
        ...
        # env.recorder_manager._terms["state"]._current_state = torch.cat([robot.data.joint_pos[:, :-1], 
        #                                                                  robot.data.root_lin_vel_b[:, :2], 
        #                                                                  robot.data.root_ang_vel_b], dim=1)
        
        env.recorder_manager._terms["state"]._current_state = torch.cat([robot.data.joint_pos[:, :-1], 
                                                                         root_state], dim=1) # root state [pos, quat, lin vel, ang vel]
        # record pre env step
        ...
        if env_cfg.write_image_to_file:
            save_images_to_file(camera_wrist_image.clone(), f"./images_vel/wrist/wrist_img_{image_save_counter}.png")
            save_images_to_file(camera_base_image.clone(), f"./images_vel/base/base_img_{image_save_counter}.png")
