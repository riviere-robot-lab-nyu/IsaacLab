...
from .rrl_m3_vel_cabinet_camera_env_cfg import M3VelCabinetCameraEnvCfg  # isort: skip


class M3VelCabinetCameraEnv(DirectRLEnv):
    ...
    cfg: M3VelCabinetCameraEnvCfg
    
    def __init__(self, cfg: M3VelCabinetCameraEnvCfg, render_mode: str | None = None, **kwargs):
       ...
       # self._setup_thrusters()
        ...
        self._vels = torch.zeros((self.num_envs, 3), device=self.device)
    ...
    def _setup_scene(self):
        ...
        # Spawn robot and cabinet
        self.robot = Articulation(self.cfg.robot)
        self.cabinet = Articulation(self.cfg.cabinet)

        # Spawn cameras
        self._tiled_camera_wrist = TiledCamera(self.cfg.tiled_camera_wrist)
        self._tiled_camera_base = TiledCamera(self.cfg.tiled_camera_base)

        # add articulation and sensors to scene
        self.scene.articulations["robot"] = self.robot
        self.scene.articulations["cabinet"] = self.cabinet
        self.scene.sensors["tiled_camera_wrist"] = self._tiled_camera_wrist
        self.scene.sensors["tiled_camera_base"] = self._tiled_camera_base
    ...
    def _pre_physics_step(self, actions: torch.Tensor): 
        ...
        # Map desired velocity (actions [-3:]) to body forces
        # Calculate individual thrusts, clamp, apply
        lin_vel = self.robot.data.root_lin_vel_b
        ang_vel = self.robot.data.root_ang_vel_b 

        self._vels[:, 0] = lin_vel[:, 0]
        self._vels[:, 1] = lin_vel[:, 1]
        self._vels[:, 2] = ang_vel[:, -1]

        err = actions[:, 7:] - self._vels
        wrench = self.kps * err
        thrust = wrench @ self.D_inv_T
        ...
        arm_targets = (
            ...
            + self.cfg.arm_speed_scale * self.dt * self._actions[:, :6] * self.cfg.action_scale
        )
        ...
        close_mask = (actions[:, 6] < 0.0).unsqueeze(-1)  # (num_envs, 1)
